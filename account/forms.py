...
class RegistrationForm(UserCreationForm):
    ...
    class Meta:
        model = Account
        fields = ('uniqueID', 'password1', 'password2', )
...
class AccountAuthenticationForm(forms.ModelForm):
    ...
    class Meta:
        model = Account
        fields = ('uniqueID', 'password')
        widgets = {
            'uniqueID': forms.TextInput(attrs={'class': 'form-control'}),
    ...
    def clean(self):
        if self.is_valid():
            ...
            if not authenticate(uniqueID=uniqueID, password=password):
